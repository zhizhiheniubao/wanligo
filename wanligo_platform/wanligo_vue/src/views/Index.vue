<template>
  <div class="wrapper infinite-list-wrapper">
      <!-- header部分 -->
      <header>
      ...
      <div class="search">
          ...
          <div class="search-fixed-top" ref="fixedBox">
              ...
              <div class="search-box">
                  <i class="fa fa-search" @click="init"></i>
                  <el-input v-model="searchForm.name" placeholder="搜索饿了么商家、商品名称"></el-input>
              </div>
          </div>
      </div>
      <!-- 点餐分类部分 -->
      <ul class="foodtype">
          <li v-for="item in categoryArr" :key="item.categoryId" @click="toBusinessList(item.categoryId)">
      ...
      <!-- 推荐商家列表部分 -->
      <ul class="business" style="overflow: auto">
              ...
              <div class="business-info">
                  ...
                  <div class="business-info-star">
                      <div class="business-info-star-left">
                          <el-rate v-model="item.grade" disabled text-color="#ff9900">
                          </el-rate>
                          <p>{{ item.grade }} 月售345单</p>
                      </div>
...
const load = () => {
  post('/business/list', {
      businessName: searchForm.value.name,
      current: current.value,
      size: size.value
  }).then(response => {
      let result = response.data.data.businessList.records
      console.log(result.length);
      if (result.length > 0) {
          for (const item in result) {
              businessArr.value.push(result[item])
          }
      }
...
const init = () => {
  ...
  get('/category/list').then(response => {
      categoryArr.value = response.data.data.categoryList
  }).catch(e => { console.log(e); })
  console.log(searchForm.value.name, current.value, size.value)
  post('/business/list', {
      businessName: searchForm.value.name,
      current: current.value,
      size: size.value
  }).then(response => {
      businessArr.value = response.data.data.businessList.records
  }).catch(e => { console.log(e); })
}
init();
...
onMounted(() => {
  ...
  document.onscroll = () => {
      ...
      let scrollHeight = document.documentElement.scrollHeight - nowScotop * 0.14 || document.body.scrollHeight - nowScotop * 0.14 //document的滚动高度
      let wheight = document.documentElement.scrollTop || document.body.scrollTop//已滚动高度
      if (onFetching.value) {
          // do nothing
      } else {
          if (scrollHeight - wheight < nowScotop) {
