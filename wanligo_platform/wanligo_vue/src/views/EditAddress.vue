<template>
    <div class="wrapper">
      ...
      <header>
        ...
        <p style="margin-left: 16vw;">编辑送货地址</p>
      </header>
  ...
  const init = () => {
    //通过daId配送地址id，获得配送地址详细信息赋值给 deliveryAddress.value 在页面进行回显
    let url = '/deliveryaddress/info/'+daId
    get(url).then(response=>{
      deliveryAddress.value = response.data.data.address
  ...
  const editAddress = () => {
    ...
    //调用服务器端，更新配送地址数据
    post('/deliveryaddress/update',deliveryAddress.value,true).then(response=>{
